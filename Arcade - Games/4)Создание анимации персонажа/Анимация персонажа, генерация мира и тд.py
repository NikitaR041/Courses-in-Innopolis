...
class MyGame(arcade.Window):
    ...
    def setup(self):
        ...
        '''Создаем Old_man'''
        self.oldman = OldMan()
        self.oldman.center_x = 250
        self.oldman.bottom = self.ground_list[0].top  # ноги персонажа будут ровно касаться поверхности земли
        '''Создаем физику'''
...
class OldMan(arcade.Sprite):
    def __init__(self):
        ...
        #Списки для храненния изображений стойки
        self.idle_textures = []
        for i in range(1,9):
            texture = load_texture_pair(f"resources/person/oldman-idle/oldman-idle-{i}.png")
            self.idle_textures.append(texture)
        self.texture = self.idle_textures[0][0]  # стандартно персонаж стоит под первой картинкой и смотрит вправо

    def update_animation(self, delta_time: float = 1 / 60):
        self.cur_texture += 0.1 #Скорость изменения картинок - счетчик
        if self.cur_texture >= 8:
            self.cur_texture = 0
        self.texture = self.idle_textures[int(self.cur_texture)][0]
        # self.texture = self.idle_textures[0][0]  # стандартно персонаж стоит под первой картинкой и смотрит вправо
